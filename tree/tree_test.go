...
func TestTreeSingleNode(t *testing.T) {
	fs := memfs.New()
	root := WrapNode(false, "/", String(Named("test.txt"), "abc123"))

	tree, err := New(fs, false, "/prefix", root.Node)
...
func TestTreeNilRoot(t *testing.T) {
	fs := memfs.New()
	tree, err := New(fs, false, "/prefix", nil)
	require.NoError(t, err)
	assert.Equal(t, &Tree{
		Filesystem: fs,
		Path:       "/prefix",
...
func TestTreeDirsAliases(t *testing.T) {
	fs := memfs.New()
	root := WrapNode(true, "/",
		Dir(Named(""),
			String(Named("test1.txt"), "abc123"),
			Dir(Named("sub", Alias("Sub")),
				String(Named("test2.txt"), "def456"))))

	tree, err := New(fs, true, "/prefix", root.Node)
	require.NoError(t, err)
	assert.Equal(t, &Tree{
		Filesystem: fs,
		Path:       "/prefix",
